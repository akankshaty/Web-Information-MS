...
<body>
	...
	<?php 
		...
		if (isset($_GET['access'])) {
			$res = mysqli_query($conn,"SELECT * FROM coordinator_invitations WHERE access_token='".$_GET['access']."'");
			...
			if (!isset($_GET['registration'])) {
				if (mysqli_num_rows($res) > 0) {
					...
					echo '<form id="survey_form" action="" method="POST">
						<h1>DR Course Coordinator Signup Form</h1>
						<table>
							<tr>
	...
	<?php 
		...
		if (isset($_GET['registration']) && isset($_GET['access']) && $access_token_valid) {
			if($_GET['registration'] == "success") {
				...
				mysqli_query($conn, "DELETE FROM coordinator_invitations WHERE email='".$email."'"); // Delete coordinator from coordinator_invitations table
				$_SESSION = array();
